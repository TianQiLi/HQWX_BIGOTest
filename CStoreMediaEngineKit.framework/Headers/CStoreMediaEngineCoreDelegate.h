...
@class CSMVideoStats, CSMAudioStats;
...
/**
 ...
 @param errorCode 错误码, 详见 CSMErrorCode
 */
 ...
 */
- (void)mediaEngine:(CStoreMediaEngineCore *)mediaEngine
didClientRoleChanged:(CSMClientRole)oldRole
            newRole:(CSMClientRole)newRole
     clientRoleInfo:(CSMChannelMicUser *)clientRoleInfo
        channelName:(NSString *)channelName;

/**
 有主播进入频道或频道内观众切换成主播时，频道的内主播和观众都会收到此回调
 
 @param mediaEngine CStoreMediaEngineCore对象
 @param user 频道内新的主播身份的用户信息
 @param elapsed 暂时没用，为0
 */
...
/**
 ...
 @param mediaEngine CStoreMediaEngineCore对象
 @param user 退出频道的主播用户信息，或切换成观众的用户信息
 @param reason 暂时没用，为0
 */
...
/**
 进入房间所携带的token或者调用renewToken更新的token存在过期时间，当离过期时间还有30s的时候触发该回调
...
/**
 ...
 - 1 : SDK销毁完毕
 */
- (void)mediaEngine:(CStoreMediaEngineCore *)mediaEngine mediaEngineChangeState:(int)state;
#pragma mark - Media
...
/**
 ...
 @param mediaEngine CStoreMediaEngineCore对象
 @param elapsed  从joinChannel开始到回调的耗时，单位ms
 */
- (void)mediaEngine:(CStoreMediaEngineCore *)mediaEngine firstLocalAudioPkgDidSend:(uint64_t)elapsed;
...
/**
 ...
 @param mediaEngine CStoreMediaEngineCore对象
 @param elapsed  从joinChannel开始到回调的耗时
 */
- (void)mediaEngine:(CStoreMediaEngineCore *)mediaEngine firstLocalVideoPkgDidSend:(uint64_t)elapsed;
...
/**
 * 美颜获取人脸数量接口的回调，返回调用getFaceNum时那一帧的人脸数量
 * @param nums 调用getFaceNum时那一帧的人脸数量。
 * 如果接口返回值小于0，表示错误码，本次检测结果可以跳过
 * -1 表示 venus未初始化
 * -3 表示 venus正在初始化
 */
- (void)mediaEngine:(CStoreMediaEngineCore * _Nonnull)mediaEngine onFaceNumGot:(int)nums;
...
@param reason 状态值
 ...
 - BigoAudioMixingErrorInterruptedEOF = 703, state为BigoAudioMixingStateStopped时，表示打开文件异常
 - BigoAudioMixingPlayEndOfFile：state为BigoAudioMixingStateStopped时，表示文件播放结束
 - BigoAudioMixingPlayActiveStop：主动停止播放
 - 其他state值，reason为毫秒ms值
*/
...
/**
 SDK 通知将要停止采集视频帧，即App收到该回调后向 SDK 发送的视频帧数据无效
...
/**
 本地视频流统计信息回调。建议使用 -mediaEngine:videoRoomStats:
...
/**
 ...
 @param mediaEngine CStoreMediaEngineCore对象
 @param stats 本地视频的统计信息: CSMVideoStats
 */
- (void)mediaEngine:(CStoreMediaEngineCore *_Nonnull)mediaEngine videoRoomStats:(CSMVideoStats *_Nonnull)stats;
...
/**
 ...
 报告更新本地音频统计信息，该回调方法每3秒触发一次。
 
 @param mediaEngine CStoreMediaEngineCore对象
 @param stats 本地音频的统计信息: CSMAudioStats
 */
- (void)mediaEngine:(CStoreMediaEngineCore *_Nonnull)mediaEngine audioRoomStats:(CSMAudioStats *_Nonnull)stats;
@end
